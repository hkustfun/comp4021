...
const Sprite = function(ctx, x, y) {
    ...
    const drawSprite = function() {
        ...
        ctx.drawImage(
            ...
            sequence.x + index * sequence.width,  // The x position of the current frame
            sequence.y,                           // The y position of the current frame
            sequence.width,                       // The width of the sprite in the sheet
            sequence.height,                      // The height of the sprite in the sheet
            x - size.width / 2,                   // The x position to draw the sprite
            y - size.height / 2,                  // The y position to draw the sprite
            size.width,                           // The scaled width
            size.height                           // The scaled height
        );
    ...
    const update = function(time) {
        ...
        if (time - lastUpdate > sequence.timing) {
            index = (index + 1) % sequence.count;  // Move to the next sprite frame
            lastUpdate = time;
    
            // If the sequence is not looped, stop at the last frame
